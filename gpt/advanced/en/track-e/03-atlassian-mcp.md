# E3 — Atlassian with the Codex CLI (Jira & Confluence)

⏱️ **15 minutes** · Track: 🅴 Tools & MCP · Needs: an Atlassian account, the Codex CLI

🌐 [Polski](../../pl/track-e/03-atlassian-mcp.md) · [← Prev](02-github-mcp.md) · [Track index](../README.md) · [Next: Azure with Codex →](04-azure-mcp.md)

---

## 🧠 Theory (4 min)

The **Atlassian MCP server** connects GPT to **Jira** (issues, boards, sprints) and **Confluence** (pages, docs). Atlassian publishes a remote MCP server you add to the Codex CLI and authorize with OAuth.
...
> Atlassian's remote MCP server and its exact URL are provided by Atlassian and may change. Check the current **"Atlassian Remote MCP Server"** docs for the endpoint, then add it to the Codex CLI.
...
Add Atlassian's MCP server to Codex (use the current URL/command from Atlassian's docs):

```powershell
codex mcp add atlassian -- <command or URL from Atlassian's docs>
```

Complete the OAuth sign-in it prompts for; approve access to your Jira/Confluence site.

> You can also add it under `[mcp_servers.atlassian]` in `~/.codex/config.toml`. Run `codex mcp --help` for the remote-server (URL) options.
...
GPT pulls live issues and gives you a stand-up-ready summary.
...
GPT drafts it, **asks first**, then files it and returns the key/link.
...
- The **Atlassian MCP server** connects the Codex CLI to **Jira** and **Confluence** — add it and authorize with OAuth.
