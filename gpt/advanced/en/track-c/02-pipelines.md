...
```
read input  →  ask GPT  →  save output
```
...
| Process each | a function that calls GPT |
| Save outputs | `write_text()` to an output folder |
| (Optional) combine | one more GPT call over all results |

> ⚠️ **Cost & safety:** batch jobs make many API calls — start with a few files and `gpt-5-mini`. Never point a pipeline at a folder with secrets or private data.
...
- A pipeline = **read → ask GPT → save**, in a loop over many items.
...
- Start small and cheap; never batch over private data.
