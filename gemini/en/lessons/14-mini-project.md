...
🌐 [Polski](../../pl/lessons/14-mini-projekt.md) · [← Prev](13-gemini-cli.md) · [Course home](../README.md) · [Next: Grounding →](15-grounding-google-search.md)

---

## 🧠 Theory (2 min)

Time to build something you'll keep: a **command-line notes assistant**. You write messy notes; Gemini cleans them up, summarizes them, and pulls out action items. This ties together Lessons 8–10.

New concept: a **system instruction**. It's a standing rule that shapes how Gemini behaves for the whole request — like the Gems from Lesson 7, but in code.
...
# A "system instruction" sets Gemini's role and rules for the whole request.
...
Gemini returns a clean summary, key points, and a checklist. ✅
...
|---------|----------------|
| **System instruction** (`system_instruction`) | Sets Gemini's role and output format once, reliably |
...
## 🚀 Make it yours (optional, use Gemini CLI!)

Open this project in **Gemini CLI** (Lesson 13) and ask for upgrades in plain English:

```text
Change notes_assistant.py so it reads notes from a file called notes.txt instead of pasting.
```
```text
Save Gemini's cleaned-up result to a file called summary.md with today's date in the filename.
```
```text
...
- [ ] You understand what the `system_instruction` does.
- [ ] (Bonus) You added at least one feature via Gemini CLI.
...
- A **system instruction** reliably shapes Gemini's role and output format.
- Asking for a fixed structure (sections/checklists) makes results reusable.
- You can grow a small script into a real tool by describing features to Gemini CLI.
