# E4 — Azure with the Gemini CLI

⏱️ **15 minutes** · Track: 🅴 Tools & MCP · Needs: an Azure account, Node.js + Azure CLI, the Gemini CLI
...
The **Azure MCP server** lets Gemini explore and work with your **Azure cloud resources** — storage, databases, resource groups, Key Vault, logs — through natural language.
...
- **Your permissions apply** — Gemini can only touch what your Azure account already can.
...
Add it to the Gemini CLI:

```powershell
gemini mcp add azure -- npx -y @azure/mcp@latest server start
```

> Or add the same under `mcpServers` in `.gemini/settings.json`. Restart the CLI so it launches the server.
...
Gemini runs Azure tools locally and returns live results — a guided tour of your cloud.
...
- For any change, make Gemini **explain what it will run and why first**, and confirm the target.
...
- [ ] `az login` works and the Azure MCP server is added to the Gemini CLI.
- [ ] Gemini listed your subscriptions/resource groups.
...
Give Gemini a read-only tour of one subscription: list resource groups, inventory the biggest, and have it produce a short plain-English summary of what you're running. Make no changes.
...
- Add it to the Gemini CLI, then query your cloud in plain English.
- Cloud changes are **high-stakes** — stay read-only, and make Gemini explain any write before it happens.
