...
Our plan: a small Python script asks Gemini for a daily brief, and Task Scheduler runs it **every morning**, saving the result to a file. You wake up, the brief is there.

The scheduled task runs **as your Windows user**, so it can read the `GEMINI_API_KEY` you set with `setx`.
...
schtasks /create /tn "GeminiDailyBrief" `
  /tr "'C:\Path\To\python.exe' 'C:\Users\YourName\learn-ai-gemini\daily_brief.py'" `
  /sc daily /st 08:00
```
...
| Runs as your user | Can read `GEMINI_API_KEY` |
