# C3 — Multi-File Projects with Gemini CLI

⏱️ **15 minutes** · Track: 🅲 Automator · Needs: Gemini CLI (beginner Lesson 13)
...
We'll have Gemini CLI build a small multi-file tool: a **CLI that turns a folder of notes into an HTML page**.
...
and builds a single index.html listing them, each with a Gemini-generated
...
Gemini CLI creates several files and shows each for approval. **Read each** before saying yes.
...
Each request touches whatever files are needed; Gemini CLI keeps them consistent.
...
> **Safety:** Gemini CLI asks before creating/editing files or running commands. On a real project, work in a **dedicated folder** (or a git branch) so changes are easy to review and undo.

---

## ✅ Checkpoint

- [ ] Gemini CLI proposed a **plan** before writing files.
...
Pick a small real tool you'd find useful (a habit tracker, a link organizer, a flashcard maker) and build it with Gemini CLI using the plan-first workflow. Keep requests small, review every change, end by asking for a README.

---

## 💡 Key takeaways

- Gemini CLI manages **multi-file projects** — creating and editing many files consistently.
